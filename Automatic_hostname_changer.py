...
CONFIG = {
    "SHARED_FOLDER": r"\\ip\de_la\carpeta_compartida",  # Cambiar IP/ruta real
    "CREDENCIALES": {
        "username": r".\Administrator",  # Usuario local
        "password": "Contraseña_del_administrador_local",   # Contraseña local
    },
    "DOMINIO_CORPORATIVO": "dominio.compartivo.corp" # Dominio corporativo
}
...
def verificar_hostname_en_ad(hostname, username, password, domain_controller, logger):
    try:
        correct_username = username + "@dominio.corporativo.corp" # Se corrige el username para SIMPLE (corregir dominio)
...
# Función cambiar_hostname corregida
def cambiar_hostname(nuevo_hostname, domain_user, domain_password, logger):
    try:
        ...
        if resultado.returncode == 0:
            ...
            # Esperar unos segundos para que se propague el cambio
            logger.info("Esperando 30 segundos para propagación en AD...")
            time.sleep(30)
...
def actualizar_json_hostnames(hostname_actual, nuevo_hostname, hostnames_data, tipo, logger):
    try:
        # PASO 1: Eliminar hostname antiguo
        eliminado = False
            ...
            for puesto in list(hostnames_data["Hostname_existentes"][lugar].keys()):
                for cat in ["PC", "LP", "Desconocido"]:
                    if cat in hostnames_data["Hostname_existentes"][lugar][puesto]:
                        ...
                        if hostname_actual in lista:
                            lista.remove(hostname_actual)
                            eliminado = True
                            logger.info(f"Eliminado de {lugar}/{puesto}/{cat}")
        
        if not eliminado:
            logger.warning("Hostname antiguo no encontrado en JSON")
        
        # PASO 2: Agregar nuevo hostname
        ...
        logger.info(f"Agregado a {lugar}/{puesto}/{cat}")
        
        return hostnames_data
    except Exception as e:
        logger.error(f"Error actualizando JSON: {str(e)}")
        return hostnames_data
...
def main():
    ...
    try:
        ...
        # Paso 8: Buscar número disponible
        ...
        # Paso 9: Construir nuevo hostname
        nuevo_hostname = f"{lugar}{puesto}{nuevo_numero}{tipo}"
        logger.info(f"Nuevo hostname propuesto: {nuevo_hostname}")
        
        # Paso 10: Cambiar hostname
        if len(sys.argv) == 3:
            usuario = sys.argv[1] #Los argumentos se pasan al script
            password = sys.argv[2] #Solamente los que tienen permisos de administrar computadoras en el AD pueden cambiar el hostname

            # 1. Primero validar credenciales
            ...
            if not verificar_hostname_en_ad(nuevo_hostname, usuario, password, CONFIG["DOMINIO_CORPORATIVO"], logger):
                logger.error(f"El hostname {nuevo_hostname} ya existe en AD")
                return EXIT_HOSTNAME_DUPLICADO
            
            # 3. Intentar cambio de hostname
            if cambiar_hostname(nuevo_hostname, usuario,password, logger):
                # Paso 11: Actualizar JSON
                hostnames_data = actualizar_json_hostnames(
                    hostname_actual,
                    nuevo_hostname,
                    hostnames_data,
                    tipo,
                ...
                # Paso 12: Guardar JSON
                guardar_json_hostnames(hostnames_data, logger)
                logger.info("Proceso completado exitosamente")
                return EXIT_SUCCESS
            else:
                logger.error("Falló el cambio de hostname")
                return EXIT_CREDENTIALS_OR_CHANGE_FAIL
        else:
            logger.error("Faltan credenciales. Uso: script.exe <usuario> <password>")
            return EXIT_CREDENTIALS_OR_CHANGE_FAIL
